...
import Window from './Window-draggable'
import SettingsApp from './apps/SettingsApp-simple'

function Desktop() {
  const [windows, setWindows] = useState([])
  const [nextZIndex, setNextZIndex] = useState(100)

  const handleOpenApp = (appType, title) => {
    console.log('Opening app:', appType, title)
    const newWindow = {
      id: Date.now(),
      appType,
      title,
  ...
  const renderAppContent = (appType) => {
    switch (appType) {
      case 'settings':
        return <SettingsApp />
      case 'fileexplorer':
        return <div><h2>File Explorer</h2><p>File explorer content here</p></div>
      case 'calculator':
        return <div><h2>Calculator</h2><p>Calculator content here</p></div>
      default:
      ...
      {windows.map(window => (
        !window.isMinimized && (
          <Window
            ...
            onFocus={() => handleWindowClick(window.id)}
